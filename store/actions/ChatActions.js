...
export const newChatRoom = chatroomName => {
   return async (dispatch, getState) => {
      ...
      const response = await fetch(
         'https://kvaliapp-15741-default-rtdb.europe-west1.firebasedatabase.app/chatrooms.json?auth=' +
            token,
         {
            method: 'POST',
            headers: {
...
export const newChatMessage = (chatRoomId, message) => {
   return async (dispatch, getState) => {
      ...
      // You must use YOUR link and not this link, to save data in your database and not mine.
      const response = await fetch(
         'https://kvaliapp-15741-default-rtdb.europe-west1.firebasedatabase.app/chatrooms' +
            chatRoomId +
            '/messages.json?auth=' +
            token,
         {
